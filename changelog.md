...
Used to record meaningful feature changes, bug fixes, important refactors, and engineering updates in NaviDash.

Recording rules:
- Grouped by date
- One entry per meaningful change
- Prioritize keeping entries concise, searchable, and traceable

## 2026-08-03

### fix: 主页打开时备忘录不再抢占焦点
- 做了什么：空备忘录首次渲染时保持输入区可见但不再自动聚焦，只有用户实际点击备忘录后才进入编辑并聚焦输入框
- 影响范围：首页初始焦点、画布键盘快速启动器、备忘录编辑交互
- 涉及模块：`src/components/widgets/MemoWidget.tsx`、`src/__tests__/MemoWidget.test.tsx`
- 是否有兼容性影响：无；不改变备忘录内容和持久化结构
- 后续待补充：无
...
- 影响范围：GitHub 项目首页、首次了解项目、部署与使用文档入口
- 涉及模块：`README.md`、`README_EN.md`、`public/navidash-demo-cover.png`
- 是否有兼容性影响：无；仅调整公开文档展示，不改变产品行为和部署方式
- 后续待补充：发布新版本时同步更新 README 版本徽章与封面内容
