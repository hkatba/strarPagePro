...
/**
 * DragDropProvider
 * Provides global drag-and-drop context, managing drag functionality from sidebar to main canvas
 */
export default function DragDropProvider({ children }: DragDropProviderProps) {
  ...
  const dragLagModifier: Modifier = useCallback(({ transform }) => {
    ...
    // Low-pass filter: gives slight lag to card following to avoid harsh instantaneous snapping
  ...
  const sensors = useSensors(
    useSensor(PointerSensor, {
      activationConstraint: {
        distance: 8, // Require 8px movement before dragging starts to prevent accidental triggers
      },
    }),
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    ...
    if (dragData) {
      ...
      // Collapse widget store automatically on drag start to free up space on main canvas
      closeSidebar();
    }
  }, [closeSidebar]);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    // Dispatch regardless of grid hit: if missed, main canvas falls back to appending at the end
